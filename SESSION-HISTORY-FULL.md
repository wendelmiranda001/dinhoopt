# DiNho Optimizer — Histórico de Sessões (arquivo)

Este arquivo contém o log detalhado, sessão a sessão, extraído do antigo `AGENTS.md` monolítico em 2026-09-12 (higienização).

**Como usar:** isto é referência histórica, não instruções ativas. As instruções do agente, o stack, os princípios e o status atual/trabalho em andamento agora vivem em `AGENTS.md`. Use este arquivo para: (1) verificar se um bug específico já foi investigado/corrigido antes de reabrir uma investigação; (2) recuperar o contexto de uma decisão técnica antiga (ex.: por que um valor de configuração tem esse default).

**Nota sobre ordem:** o arquivo original tinha algumas entradas fora de ordem cronológica (ex.: um resumo datado de 2026-08-17 aparecia depois de sessões de 2026-08-23) — essa ordem foi preservada aqui exatamente como estava no original, sem tentar corrigi-la, para não alterar o conteúdo de ninguém.
