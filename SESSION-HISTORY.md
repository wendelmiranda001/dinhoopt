...
- **2026-09-16** — **Auditoria de memória C# — relatório superestimou; gaps reais corrigidos em `EngineCoordinator.Dispose`**
    - Investigação de 1 dia (relatório em `Desktop\investigacao-diNho-clips.md`): a verificação linha-a-linha de todos os timers/threads/processos/handlers C# mostrou que **a maioria dos "8 vazamentos críticos" do relatório preliminar JÁ era tratada** — timers descartados (`StopAsync`, `StopCapture`, `GameDetector.Stop`, `NamedPipeServer.Stop`, `RamManager.StopWatchdog`, `EngineStatus.Dispose`), threads com `Join` (FfmpegEncoder/FrameWriter/WindowsMessagePump/HotkeyManager/GameDetector/CppLoopbackSource), processos com kill (Maxine/Rnnoise/FfmpegAac/Ffmpeg/ClipExporter), handlers WASAPI mortos junto com o objeto.
    - **Gap 1:** `Dispose()` não descartava `_cleanupTimer` (só `StopAsync()` fazia) → timer podia ficar vivo.
    - **Gap 2:** **7 eventos** assinados no ctor nunca eram desassinados no `Dispose()` — `_config.OnConfigChanged`, `_hotkeys.OnHotkeyPressed`, `_gameDetector.OnGameChanged`, `_ptt.OnMicStateChanged`, `_pipeServer.OnMessage/GetStatus/OnStatusBroadcast`. O lambda do config foi movido para o campo `_onConfigChangedHandler` para permitir `-=`.
    - **Gap 3:** `Dispose()` não era null-safe (`_hotkeys.Dispose()`, `_audioSessions.Dispose()` etc. — NRE em objetos parcialmente inicializados); agora tudo `?.`/`is not null`.
    - **Testes (TDD RED→GREEN):** novo `EngineCoordinatorDisposeTests.cs` (3 testes: `Dispose_ClearsAndDisposesCleanupTimer`, `Dispose_UnsubscribesEventHandlersWiredInConstructor`, `Dispose_WithNullModules_DoesNotThrow`), usando `GetUninitializedObject` + reflexão + `Delegate.CreateDelegate` para reproduzir o wiring do ctor sem tocar HW (`AudioSessionManager` não é injetável).
    - Suíte **1582/1582 GREEN** (1 falha isolada de `VideoPacketPool` = flakiness de estado estático paralelo, passa isolada e na 2ª rodada). Commit `3e002ea`.
- **2026-09-16b** — **Avaliação das 9 sugestões de backend (`sugestoes-avaliacao-backend-dinho-clips.md`): 2 implementadas, 7 descartadas com causa-raiz**
    - **Descartadas (verificação no código real, sem alteração):**
        - (1) "Duplicação `OnMixedAudio += OnAudioPacket`" — impossível: `StartCapture` e `HandleSetMicDevice` **sempre criam mixer novo** (`CreateAudioMixer`) antes de assinar e `if (_captureActive) return` impede Start duplo. Overlap curto no `setMicDevice` é troca deliberada por continuidade de áudio.
        - (2) "WASAPI sem `-=` no Dispose" — sem leak real: fontes são owned/descartadas por inteiro pelo `AudioMixer` a cada captura/troca de mic; delegate morre com o grafo de objetos.
        - (3) "`Process.ErrorDataReceived` nunca desinscrito" — cada instância de filtro/encoder owns UM processo longe-lived morto no Dispose (nunca reiniciado na mesma instância); `ClipExporter` usa `Process` local de método. Sem reaproveitamento ⇒ sem leak.
        - (4) "`GC.SuppressFinalize`" — **zero finalizadores** no codebase inteiro; ausência é inofensiva na prática (confirmado por busca com padrões de finalizador).
        - (5) "73× `.Result`/`.Wait()`" — todos em threads dedicadas/threadpool **sem SynchronizationContext** + bounded por timeout (2–8s); deadlock é teórico. Migração em massa = churn alto/benefício zero. Revisitar só se o engine passar a rodar em contexto de UI.
        - (6) "15× `new Thread`/`Thread.Sleep`" — todos justificados: STA/COM (GameDetectorHook), message pumps (HotkeyManager/WindowsMessagePump), readers dedicados (FrameWriter/AacReader/stderr), StopGuard bounded.
        - (9) "~161 locks" — sem evidência de contenção (FeedTelemetry: encoder queue=0); preocupação teórica. Regra mantida: só mexer com profiling que mostre contenção real.
    - **Implementado (item 8) — `WindowClassMap` morto removido:** confirmado `[Obsolete]` em `GameDetector.cs` (declarado lá, não em GameDatabase.cs), **zero referências** em src+tests, sem reflection/serialização. O caminho vivo é `KnownGames.LookupWindowClass` → `GameDatabase.GetDisplayName` (games.json + fallback `HardcodedMap` de 7 entradas). Removidos os ~80 entries + doc block stale; `STATUS.md` atualizado (passou a citar `LookupWindowClass`).
    - **Implementado (item 7) — fix de honestidade do `MaxineAfxFilter` (bug real sem HW):** `CheckMaxineAvailability` usava `nvcuda.dll`+`nvml.dll` (prova só GPU NVIDIA, não o SDK) ⇒ em RTX alegava "Maxine AFX available" e configurava `arnndn=m=models/rnnoise/model.rnnn` (**path relativo** que não resolve ⇒ ffmpeg morria ⇒ supressão de ruído **silenciosamente morta** nas máquinas que deveriam se beneficiar), enquanto não-RTX caía em `anlmdn` (funcionava). Fix: detecção agora baseada no **SDK real** (`nvaudiofx64.dll` em `AppContext.BaseDirectory`, seam puro `IsMaxineSdkPresent` para testar sem HW) e `BuildMaxineFilter` resolve o modelo p/ caminho **absoluto** (usa `arnndn` só com `model.rnnn` existente, senão `anlmdn`). 8 testes novos em `MaxineAfxFilterTests.cs` (TDD RED→GREEN).
    - Suíte **1590/1590 GREEN** (era 1582; +8). Build Debug e Release 0 erros (3 warnings pré-existentes de nullability).
- **2026-09-16c** — **Multi-Track Audio (Item 5) DESCARTADA pelo usuário — código removido**
    - Usuário descartou a feature inteira antes da implementação HW (`AudioMixer` multi-stream WASAPI, N encoders AAC, N streams ADTS→MKV). O commit `14d939b` (política pura `MultiTrackAudioPolicy.ResolveTracks` + `AudioTrackKind`, 5/5 GREEN) ficou como histórico, mas o **código morto foi removido**: `MultiTrackAudioPolicy.cs`, `AudioTrackKind.cs`, `AudioInputConfig.cs`, `MultiTrackAudioPolicyTests.cs` (suíte 1590 → 1585). **Não reabrir sem novo pedido explícito.** Registrado em `AGENTS.md` (seção "DESCARTADA pelo usuário" + lista de rejeitados).
